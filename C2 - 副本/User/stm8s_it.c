...
INTERRUPT_HANDLER(TIM1_UPD_OVF_TRG_BRK_IRQHandler, 11)
{
   ...
   */
   ...
   extern bool KEY_UP;
   extern bool DS18B20_CONVERT_STATUS;
   extern bool CONFIG_STATUS;
   extern bool KEY_DOWN;
   extern bool CONFIG_LED_STATUS;
   extern bool HF_STATUS;
   extern bool IT_HF_STATUS;
   extern bool IT_TEM_SUB_STATUS;
   extern bool IT_TEM_SUB_STATUS_M;

   static u8 LED_PWM_TIME;
   static u8 LED_PWM;
   static u16 DS18B20_CONVERT_TIME;
   static u16 CONFIG_TIME;
   static u16 KEY_DOWN_TIME;
   static u16 HF_TIME_S, TEM_SUB_TIME_S;
   static u8 HF_TIME_M, HF_TIME_H;

   static bool bol_Config_LED_status = FALSE;
   static bool the_tem_sub_status = FALSE;

   // 分段加温
   if (IT_TEM_SUB_STATUS)
   {
      if (the_tem_sub_status)
      {
         if (TEM_SUB_TIME_S > 5000)
         {
            TEM_SUB_TIME_S = 0;
            IT_TEM_SUB_STATUS_M = TRUE;
            the_tem_sub_status = FALSE;
         }
      }
      else
      {
         if (TEM_SUB_TIME_S > 3000)
         {
            TEM_SUB_TIME_S = 0;
            IT_TEM_SUB_STATUS_M = FALSE;
            the_tem_sub_status = TRUE;
         }
      }
      TEM_SUB_TIME_S++;
      TEM_SUB_TIME_S = (TEM_SUB_TIME_S > 5000) ? 5000 : TEM_SUB_TIME_S;
   }
   else
   {
      TEM_SUB_TIME_S = 0;
   ...
   if (CONFIG_STATUS)
   {
      ...
      {
         CONFIG_TIME = 0;
         bol_Config_LED_status = !bol_Config_LED_status;
         CONFIG_LED_STATUS = bol_Config_LED_status;
      }
      else
      {
         CONFIG_TIME++;
